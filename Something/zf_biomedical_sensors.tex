...
Potassium is the dominant ion (highest conductance), sodium has only a small effect, all other ion lumped together in $V_0,G_0$\\
		...
		\subsection{Hodgkin Huxley Model}
			...
			$J_m(t)=J_C+J_{Na}+J_K+J_L+J_{ext}=\\C_m\frac{dV_m(t)}{dt}+G_{Na}(V_m,t)\cdot(V_m(t)-V_{Na})+G_K(V_m,t)\cdot(V_m(t)-V_K)+G_L\cdot(V_m(t)-V_L)+J_{ext}=0$\\
			\boxed{
			\begin{aligned}
			...
			Standard parameters:$\bar{G}_{Na}=120,\bar{G}_K=36, G_L=0.3mS/cm^2,\\C_m=1\mu F/cm^2,c_{Na}^o=491,
			c_{Na}^o=20.11,c_K^i=400 mmol/L, \\V_L=-49mV, temperature=6.3^\circ C$\\
